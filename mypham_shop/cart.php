    ...
    <div class="container">
        <div class="row">
            <div class="col d-flex justify-content-end mb-2">
                <button class="btn btn-outline-dark btn-flat btn-sm" type="button" id="empty_cart">Xóa giỏ hàng</button>
            </div>
        </div>
                ...
                ?>
                    <div class="d-flex w-100 justify-content-between  mb-2 py-2 border-bottom cart-item">
                        ...
                        <div class="col text-right align-items-center d-flex justify-content-end">
                            <h4><b class="total-amount"><?php echo number_format($row['price'] * $row['quantity'])   ?> VNĐ</b></h4>
                        </div>
                    </div>
                <?php endwhile; ?>
                <div class="d-flex w-100 justify-content-between mb-2 py-2 border-bottom">
                    <div class="col-8 d-flex justify-content-end"><h4>Tổng giá:</h4></div>
...
<script>
    function calc_total(){
        ...
        $('#grand-total').text(parseFloat(total).toLocaleString('en-US')+' VNĐ')
    }
    function qty_change($type,_this){
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err)
                alert_toast("có lỗi đã xảy ra", 'Lỗi');
                end_loader()
            },
    ...
    $(function(){
        ...
        $('#empty_cart').click(function(){
            // empty_cart()
            _conf("Bạn có chắc muốn xóa tất cả sản phẩm trong giỏ hàng của mình không?",'empty_cart',[])
        })
        $('.rem_item').click(function(){
            _conf("Bạn có chắc muốn xóa tất cả sản phẩm trong giỏ hàng của mình không?",'rem_item',[$(this).attr('data-id')])
        })
    })
</script>