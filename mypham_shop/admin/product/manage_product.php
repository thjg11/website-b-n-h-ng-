...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Update ": "Tạo " ?> Sản phẩm</h3>
	</div>
	<div class="card-body">
		<form action="" id="product-form">
			<input type="hidden" name ="id" value="<?php echo isset($id) ? $id : '' ?>">
            <div class="form-group">
				<label for="category_id" class="control-label">Nhóm</label>
            ...
            <div class="form-group">
				<label for="sub_category_id" class="control-label">Hạng mục</label>
                <select name="sub_category_id" id="sub_category_id" class="custom-select" required>
                <option value="" selected="" disabled="">Chọn nhóm trước</option>
                <?php
...
<script>
    ...
    function delete_img($path){
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err)
                alert_toast("Đã xảy ra lỗi khi xóa hình ảnh","Lỗi");
                end_loader()
            },
            success:function(resp){
                ...
                if(typeof resp =='object' && resp.status == 'success'){
                    ...
                    alert_toast("Đã xóa thành công hình ảnh","Thông báo");
                }else{
                    console.log(resp)
                    alert_toast("Đã xảy ra lỗi khi xóa hình ảnh","Lỗi");
                }
	...
	$(document).ready(function(){
        $('.rem_img').click(function(){
            _conf("Đảm bảo xóa hình ảnh này vĩnh viễn?",'delete_img',["'"+$(this).attr('data-path')+"'"])
        })
        ...
        $('#category_id').change(function(){
            ...
            $('#sub_category_id').select2({placeholder:"Chọn",width:"relative"})
        })
        $('.select2').select2({placeholder:"Chọn",width:"relative"})
